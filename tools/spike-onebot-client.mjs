...
// 770001 = ExpTest：group-bot-config.json 里四个 bot 都启用，泄漏影响为零
const fakeGroup = '770001';
const fakeUser = '777001';
// 独立 self_id：不与 NapCat 的 REDACTED_QQ_002 冲突（Shiro 按 self_id 只允许一个客户端）
const spikeSelfId = 'REDACTED_QQ_003';
...
const hydrantToken = (() => {
  try {
    const cfg = JSON.parse(fs.readFileSync('REDACTED_BOTS_ROOT/configs/private/hydrant/appsettings.json', 'utf8'));
...
results.push(await probe('yumu-bp', 'ws://127.0.0.1:8388/pub/onebotSocket', { headers: roleHeaders }, groupEvent('!bp [SHK]Wuxin 1-3'), 45000));
