...
async function main() {
  ...
  try {
    ...
    db.settings.ownerQq = db.settings.ownerQq || 'REDACTED_QQ_001';
    db.settings.selfQq = db.settings.selfQq || 'REDACTED_QQ_005';
    ...
    const pureAt = await processIncoming(event({
      text: '[CQ:at,qq=REDACTED_QQ_005]',
      atTargets: ['REDACTED_QQ_005']
    }), sendMessage);
    ...
    const visualAsk = await processIncoming(event({
      text: '[CQ:at,qq=REDACTED_QQ_005] 看看这张图 [图片]',
      atTargets: ['REDACTED_QQ_005']
    }), sendMessage);
