...
const { applyRetention } = await import('../server/store.ts');

const base = { messages: [], decisions: [], commandLogs: [], adminActions: [] };
...
const small = { messages: [{ id: 'keep' }], decisions: [], commandLogs: [], adminActions: [] };
...
console.log('store retention caps passed (messages/decisions/commandLogs/adminActions)');
process.exit(0);
