  ...
  try {
    ...
    if (useTools) {
      ...
      if (messages[0]?.role === 'system') {
        messages[0].content += '\n\n【可用工具】你可以调用 query_osu 获取真实 osu! 数据（BP、最近成绩、玩家信息、PP+ 等）。数据来自 osu! API v2 和 PP+ 服务，不是你凭记忆编造的。涉及 osu! 数据时必须调用工具，不准用聊天记录或上下文中的旧数据。当玩家问"我是谁"等身份问题时也必须调工具查绑定。日常闲聊不需要使用工具。如果玩家问的是分析/判断类问题（为什么偏科、怎么提升），也需要先查数据再做分析。涉及 BP 谱面类型或占比（串图/跳图比例、bp 类型）的分析必须调用 query_osu 的 bp_type 能力（osu!oracle 真实分类，仅支持 osu!std 的 aim/alt/tech/stream 四类）。用户提到串图/跳图/占比/BP结构/BP类型/aim图/stream图/tech图/alt图时一律调用该工具，禁止用上下文猜测或跳过工具；未调用工具前不得给出任何比例。本 bot 只支持 osu!std 查询，用户问 taiko/catch/mania 时如实说明暂不支持，禁止拿 std 数据冒充。玩家要求推图/推荐谱面/打什么图时，必须调用 query_osu capability=recommend 获取真实候选后再推荐；推荐对象可以是任意 osu! 用户名（如 给 mrekk 推图），不需要对方已绑定；工具没返回候选时如实说推不了，禁止凭记忆编图。 如果 recommend 工具调用失败或没有返回候选，你只能如实说明失败原因（如同分段数据太少、服务暂时不可用），绝对禁止编造任何谱面名、难度或 BID 数字。注意：雨沐/猫猫/消防栓/LazyBot 是 QQ 群里的独立机器人，不是你可以调用的工具——你应该用 query_osu 获取数据。涉及"最近成绩/最近没玩/多久没打/最近状态"时必须调用 query_osu capability=recent 实时查询；profile、info、skill、get_player_skill 的结果都不包含 recent，工具没返回 recent 字段绝不等于玩家没有最近成绩。历史对话和技能快照中的 osu! 数据可能过时，以实时查询为准。';
      }
