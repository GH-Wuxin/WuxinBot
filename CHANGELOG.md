...
### 新增
- **经验等级系统**：5 级系统（🌱新人→💬群友→🎯活跃群友→⭐老熟人→👑核心群友），XP 全局累计按 QQ 号，每日上限 30，连续活跃加成（×1.2/×1.5/×2.0），30 天不活跃自动降级。新指令 `/w lv` `/w top` `/w nick` `/w style` `/w me`。buildPrompt 注入用户等级+称呼+风格，LLM 自然适配。升级时 LLM 生成个性化恭喜语（可关闭）。GUI 成员页显示等级。
