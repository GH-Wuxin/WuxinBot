...
import { quotaWindows } from '../Dashboard/usageView.js';
...
export function ModelsPage({ db, saveSettings }) {
  ...
  const save = async () => {
    ...
    try {
      await saveSettings(patch);
      setDirty(false);
    } catch (error) { setSaveError(error.message || '保存失败，请重试'); }
  ...
  return <div className="console-page models-page" data-section={section}>
    ...
    <div className="models-layout">
      <div className="console-setting-stack">
        <SettingGroup className="model-panel model-panel--account" title="供应商与模型" description="切换已配置过的模型系列时会复用对应接口与密钥。">
          ...
          {draft.llmProvider === 'codex-app-server' ? <>
            ...
            <details className="osu-advanced"><summary>高级连接设置<span>程序路径与自动发现</span></summary><SettingRow title="Codex 可执行文件" description="官方版本路径失效时自动发现新版本；自定义路径保持不变。" control={<Input aria-label="Codex 可执行文件" value={draft.codexExecutable || 'codex'} onChange={(event) => updateDraft({ codexExecutable: event.target.value })} />} />{codexStatus?.command && <p className="osu-path-note">当前解析路径：<code>{codexStatus.command}</code></p>}</details>
            ...
            {quotaWindows(codexLimits).map(window => <InlineHelp key={window.key}>{window.bucket} · {window.label}：{window.used == null ? '用量未知' : `已用 ${Math.round(window.used)}%`} · 重置 {window.resetsAt ? new Date(window.resetsAt * 1000).toLocaleString() : '未知'}</InlineHelp>)}
          </> : <>
      ...
      <div className="console-setting-stack">
        <SettingGroup className="model-panel model-panel--capabilities" title="视觉输入" description="图片理解需要实际多模态模型支持。">
        ...
        <SettingGroup className="model-panel model-panel--capabilities" title="联网搜索" description="Pippi 会根据问题的时效性和不确定性自行决定是否搜索；用户明确要求时则保证执行。">
        ...
        <SettingGroup className="model-panel model-panel--participation" title="主动参与" description="这些参数只影响自然/轻度模式的接话判断。">
