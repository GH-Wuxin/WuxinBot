...
import { Activity, ArrowDownLeft, ArrowUpRight, ChevronRight, Clock3, MessageCircle, RefreshCw, UsersRound, Zap } from 'lucide-react';
...
function UsageChart({ summary, period }) {
  ...
  return <Card className="osu-usage">
    <header className="osu-usage__heading"><div><span className="osu-eyebrow">ACTIVITY / 用量记录</span><h2>每一次对话，都有迹可循。</h2></div><span className="osu-period-label">{label}</span></header>
    <div className="osu-usage__score"><div><Score value={totals.totalTokens} /><span className="osu-score-unit">TOKENS</span></div><p>{formatNumber(totals.requests)} 次请求<span>当前统计周期</span></p></div>
    <div className={'osu-chart' + (period === 'daily7' ? ' osu-chart--daily' : '')}>
      {period === 'all' || !points.length ? <div className="osu-chart__empty"><Activity size={26} /><strong>{period === 'all' ? '这是一路以来的总消耗' : '等待下一次对话'}</strong><span>{period === 'all' ? '切换到 7 天或 24 小时，查看用量分布。' : '当前周期还没有用量记录。'}</span></div> : <>
    ...
    <div className="osu-chart__caption" aria-live="polite">{selected ? selected.label + ' · ' + formatNumber(selected.totalTokens) + ' Token · ' + formatNumber(selected.requests) + ' 次请求' : period === 'all' ? '累计数据保留原有统计口径' : '按时间分布 · 悬停或聚焦柱条查看详情'}</div>
...
function QuotaPanel({ enabled, onNavigate }) {
  ...
  return <Card className="osu-quota"><SectionHeader eyebrow="ACCOUNT / 账号额度" title={enabled ? 'Codex 额度' : 'API 模型通道'} actions={<Button variant="ghost" size="sm" onClick={() => onNavigate('model')}>管理<ChevronRight size={14} /></Button>} />
...
export function DashboardPage({ db, oneBot, saveSettings, refreshState, onNavigate }) {
  ...
  return <div className="dashboard-page osu-dashboard">
    <div className="osu-dashboard__intro"><div><span className="osu-eyebrow">YOUR BOT, IN RHYTHM.</span><h2>掌握每一拍<span>。</span></h2><p>对话、用量与运行状态，一眼看清。</p></div><div className="osu-dashboard__period"><span>用量统计周期</span><SegmentedControl value={period} onChange={setPeriod} label="Token 统计周期" options={usagePeriods} /></div></div>
    ...
    <div className="osu-dashboard__stage"><UsageChart summary={summary} period={period} /><aside className="osu-dashboard__rail"><QuotaPanel enabled={codexActive} onNavigate={onNavigate} /><Card className="osu-health"><SectionHeader eyebrow="LIVE / 实时连接" title="运行状态" />
    ...
    <section className="dashboard-controls"><Card><SectionHeader eyebrow="PLAY STYLE / 参与方式" title="什么时候加入聊天？" /><Switch checked={Boolean(db.settings.onlyMentionMode)} onChange={event => saveSettings({ onlyMentionMode: event.target.checked })} label="临时只在 @ 时回复" description="立即生效；关闭后恢复各群自己的参与模式。" /></Card><Card><SectionHeader eyebrow="QUICK SELECT / 模型" title="选择这一拍的搭档" actions={<Button variant="ghost" size="sm" onClick={() => onNavigate('model')}>设置<ChevronRight size={14} /></Button>} /><SegmentedControl value={effectiveModel} onChange={model => saveSettings(codexActive ? { codexModel: model } : { model })} label="快速切换模型" options={modelOptions} /></Card></section>
  </div>;
}
